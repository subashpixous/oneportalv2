...
export class MemberApprovalHistoryComponent {
  ...
  actions: Actions[] = [];
  title: string = 'Member Approval History';
  first: number = 0;
  rows: number = 10;
  total: number = 0;
...
onExportRequest(type: string) {
  ...
  // 🟢 Call background export endpoint
  this.userService.startMemberApprovalExport(exportFilter, type).subscribe({
    next: (res: any) => {
      const downloadUrl = res.downloadUrl;

      // ✅ Use <a> tag instead of iframe (clean + safe)
    ...
    error: (err) => {
      ...
      this.messageService.add({
        severity: 'error',
        summary: 'Error',
        detail: 'Failed to start export. Please try again later.',
        life: 3000
      });
      this.isExporting = false;
    }
