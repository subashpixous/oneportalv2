...
export class MemberUpdateApprovalComponent {
  ...
  ngOnInit() {
    ...
    // Altered by Elanjsuriyan for deleting approval - 12-12-2025
    ...
    console.log('Approval Form', this.approvalformDetail);
  ...
  changeApproveStatus() {
    ...
    var appstats: ApproveStatusItemModel | undefined = this.approvalStatuses.find((x) => x.statusId == selectedstsId);
  ...
  submitfnAlone() {
    ...
    if (currentUrl.includes('delete-approval') && selectedText.toLowerCase().startsWith('approve')) {
      selectedText = selectedText.replace(/^Approve/i, 'Delete');
